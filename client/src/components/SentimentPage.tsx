        ...
        >
          ...
          {isExpanded && !isEditing && (
            <div className="px-4 pb-3 border-t border-gray-100">
              ...
              {/* Favorite patterns section */}
              {tickerFavorites.length > 0 && (
                <div className="mt-3 pt-2 border-t border-gray-100">
                  <div className="flex items-center gap-1.5 mb-2">
                    <svg className="w-4 h-4 text-pink-500" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                    </svg>
                    <span className="text-xs font-medium text-gray-600">Favorite Patterns</span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {tickerFavorites.map((fav) => (
                      <button
                        key={`${fav.month}-${fav.holdingPeriod}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          if (onSelectPattern) {
                            onSelectPattern(item.ticker, fav.month, fav.holdingPeriod);
                          }
                        }}
                        className="px-2 py-1 text-xs bg-pink-50 text-pink-700 rounded-md hover:bg-pink-100 transition-colors border border-pink-200"
                        title={`View ${fav.monthName} pattern (${fav.holdingPeriod}-day hold)`}
                      >
                        {fav.monthName} ({fav.holdingPeriod}d)
                      </button>
                    ))}
                  </div>
                </div>
