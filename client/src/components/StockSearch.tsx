...
export function StockSearch({ onSelect, selected, size = 'compact', autoFocus = false, variant = 'default' }: StockSearchProps) {
      ...
      <div className={`flex items-center gap-2 ${isSidebar ? 'w-full' : ''}`}>
        ...
        <div className={`relative ${isLarge || isSidebar ? 'w-full' : ''}`}>
          <input
            ref={inputRef}
            type="text"
            placeholder={isSidebar ? "Search ticker..." : isLarge ? "Search for a stock ticker (e.g., AAPL, TSLA, NVDA)..." : "Search stocks..."}
            value={query}
            onChange={(e) => {
      ...
      {isOpen && (query.length > 0 || results.length > 0) && (
        <div
          ...
          className={`absolute top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto ${
            isLarge || isSidebar ? 'w-full left-0' : 'right-0 w-72'
          }`}
        >
          {isLoading ? (
            ...
            results.map((stock) => (
              ...
              >
                <div>
                  <div className={`font-semibold text-gray-900 ${isLarge ? 'text-lg' : ''}`}>
                    {stock.ticker}
                  </div>
